...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/05/06 14:09:39 by adenece           #+#    #+#             */
/*   Updated: 2016/05/12 16:38:30 by adenece          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		get_line(char **array, char **line, int len)
{
	char	*tmp;
...
static int		ft_read_all(int fd, char **array, char **line)
{
	...
	while ((ret = read(fd, buf, BUFF_SIZE)))
	{
		...
		*array = ft_strjoin(*array, buf);
		...
		if ((len = find_nl(*array)) > -1)
			return (get_line(array, line, len));
	}
	return (-2);
}
...
int				get_next_line(int fd, char **line)
{
	...
	if (ft_strlen(array[fd]) && (len = find_nl(array[fd])) != -1)
		return (get_line(&array[fd], line, len));
	if ((ret = ft_read_all(fd, &array[fd], line)) && ret != -2)
		return (ret);
